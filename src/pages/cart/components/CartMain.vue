...
const getMemberCartData = async () => {
  ...
  memberCartList.value = res.result
}
...
// 选中事件
const selectedCart = async (item: CartItem) => {
...
const isSelectedAll = computed(
  () => memberCartList.value?.length && memberCartList.value?.every((e) => e.selected),
)

// 全选或全不选
const onChangeSelectedAll = async () => {
  const _isSelectedAll = !isSelectedAll.value
  memberCartList.value?.forEach((v) => {
...
// 计算选中的商品列表
const selectedCartList = computed(() => memberCartList.value?.filter((item) => item.selected))

// 底部商品数量计算属性
const cartCountAll = computed(() =>
  selectedCartList.value?.reduce((pre, item) => pre + item.count, 0),
)

// 底部商品金额计算属性
const cartPriceAll = computed(() =>
  selectedCartList.value?.reduce((pre, item) => pre + item.nowPrice * item.count, 0).toFixed(2),
)
  ...
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onScrollTolower">
    ...
    <template v-if="memberStore.profile">
          ...
          >
            ...
            <template #right>
              <view class="cart-swipe-right">
                <button class="button delete-button" @tap="deleteMemberCart(item.skuId)">
                  删除
                </button>
              </view>
            </template>
      ...
      <view class="toolbar">
        ...
        <text class="amount">{{ cartPriceAll }}</text>
